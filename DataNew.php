...
$webOS   = stripos($_SERVER['HTTP_USER_AGENT'],"webOS");
      ...
      $sql = $db->query("SELECT * FROM apple WHERE like_num < 120 ORDER BY id DESC LIMIT $start, $limit"); 
      $directory = 'apple/';
    } else {    
      $sql = $db->query("SELECT * FROM images WHERE like_num < 120 ORDER BY id DESC LIMIT $start, $limit");
      $directory = 'uploads/';
    }
        if ($sql->num_rows > 0) {
          $response = "";
...
// $isApple = array ($ipod, $iphone, $ipad);            
